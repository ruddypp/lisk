...
export const NFTCard = ({ tokenId }: NFTCardProps) => {
  ...
  // Get listing info
  // @ts-expect-error NFTMarketplace typing not included in Scaffold contract union yet
  const { data: listing, refetch: refetchListing } = useScaffoldContractRead({
    contractName: "NFTMarketplace" as any,
  ...
  useEffect(() => {
    if (marketplaceAddress) {
      const approvedAddr = approvedAddress as unknown as `0x${string}` | undefined;
      setIsApproved(
        approvedAddr?.toLowerCase() === (marketplaceAddress as string).toLowerCase() || isApprovedForAll === true,
      );
    }
  ...
  const { writeAsync: listItem } = useScaffoldContractWrite({
    contractName: "NFTMarketplace" as any,
    functionName: "listItem" as any,
    args: [BigInt(tokenId), parseEther(listPrice || "0")] as any,
  });

  // @ts-expect-error NFTMarketplace typing not included in Scaffold contract union yet
  const { writeAsync: buyItem } = useScaffoldContractWrite({
    contractName: "NFTMarketplace" as any,
    functionName: "buyItem" as any,
    args: [BigInt(tokenId)] as any,
  });
  ...
  const handleBuy = async () => {
    const currentListing = listing as unknown as
      | { seller: `0x${string}`; price: bigint; isActive: boolean }
      | undefined;
    if (!currentListing?.price) {
      notification.error("Invalid listing price");
      return;
    }
    try {
      await buyItem({
  ...
  const listingData = listing as unknown as { seller: `0x${string}`; price: bigint; isActive: boolean } | undefined;
  const ownerStr = (owner as unknown as string | undefined) ?? undefined;
  const isOwner = ownerStr ? ownerStr.toLowerCase() === connectedAddress?.toLowerCase() : false;
  const isListed = listingData?.isActive === true;
  const priceInEth = listingData?.price ? formatEther(listingData.price) : "0";
          ...
          {isListed && (
            <div className="stats shadow mt-2">
              <div className="stat p-4">
                ...
                {isLoadingPrice ? (
                  <div className="stat-desc">Updating price...</div>
                ) : (
